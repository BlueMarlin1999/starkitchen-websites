...
const queueDismissals = (toasts: ToasterToast[], toastId?: string) => {
  if (toastId) {
...
const dismissToasts = (state: State, toastId?: string): State => {
  queueDismissals(state.toasts, toastId)
  return {
    ...state,
    toasts: state.toasts.map((toast) => closeToast(toast, toastId)),
  }
}

const removeToasts = (state: State, toastId?: string): State =>
  toastId === undefined
    ? {
        ...state,
        toasts: [],
